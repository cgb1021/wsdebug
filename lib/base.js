...
function Base(host, port, ssl, timeout, onerror) {
  ...
  socket.addEventListener('open', function () {
    _this.send("".concat(_config.protocol.role).concat(type, "/").concat(_this.name, ":").concat(_this.password));
  ...
  socket.addEventListener('message', function (_ref) {
    ...
    if (!message.indexOf(_config.protocol.connect)) {
      ...
      try {
        arr = message.substr(_config.protocol.connect.length).split('/');
      } catch (e) {
        console.error(e);
      }

      connectedCallbacks.forEach(function (fn) {
        return fn(arr[0] ? arr[0].split(',').map(function (str) {
          if (type === 'master') {
            var _arr = str.split(':');
    ...
    if (msgId && typeof promiseCallback[msgId[1]] !== 'undefined') {
      var item = promiseCallback[msgId[1]];
      window.clearTimeout(item.timeoutId);
      item.resolve(message);
      delete promiseCallback[msgId[1]]; // console.log('delete promiseCallback[msgId[1]]', promiseCallback);
    }
...
Base.prototype.query = function () {
  return this.send2("".concat(_config.protocol.query, "1"));
};
