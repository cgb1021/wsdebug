...
function Base(host, port, ssl, timeout, onerror) {
  ...
  switch (arguments.length) {
    ...
    case 3:
      ssl = (0, _typeof2["default"])(ssl) !== 'object' && ssl ? true : false;
      break;
  ...
  socket.addEventListener('message', function (_ref2) {
    ...
    if (id && typeof promiseCallback[id] !== 'undefined') {
      ...
      if (message.indexOf(_config.protocol.error) > -1) {
        item.reject(new Error(message.substr(_config.protocol.error.length)));
      } else {
  ...
  this.setRole = function () {
    return this.send2("".concat(_config.protocol.role).concat(role, "/").concat(this.name, ":").concat(this.password)).then(function (data) {
      return data.substr(_config.protocol.role.length) === role;
    });
  };
