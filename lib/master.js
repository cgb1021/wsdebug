...
function Master() {
  ...
  this.on('open', function () {
    _this.send("".concat(_config.protocol.role, "master/").concat(_this.name, ":").concat(_this.password));
  });
  ...
  this.run = function (script, callback) {
    ...
    if (typeof callback === 'function') {
      ...
      this.send("".concat(_config.protocol.script).concat(id, "/").concat(script));
      setTimeout(function () {
        delete callbackMap[id];
      }, 180000); // 5min timeout
    } else {
      this.send("".concat(_config.protocol.script).concat(script));
    }
  };
