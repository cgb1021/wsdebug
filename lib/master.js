...
function Master() {
    ...
    onmessage: function onmessage(_ref) {
      ...
      if (!data.indexOf(_config.protocol.result)) {
        if (typeof callbackMap[id] !== 'undefined') {
          callbackMap[id](data.substr(_config.protocol.result.length));
        }
      }

      if (!data.indexOf(_config.protocol.error)) {
        var result = data.substr(_config.protocol.error.length);
        var error = new Error(result ? result : 'unknow error');
  ...
  this.run = function (script, callback) {
    ...
    var id = this.send("".concat(_config.protocol.script).concat(sessionId, "/").concat(script));
