...
function Client() {
  ...
  this.on('message', function (_ref) {
    ...
    if (!data.indexOf(_config.protocol.result)) {
      var val = data.substr(_config.protocol.result.length);
      ...
      if (match) {
        ...
        var send = function send(err, res) {
          if (err) {
            ...
            if (id) {
              _this.send("".concat(_config.protocol.error).concat(id, "/").concat(msg));
            } else {
              _this.send("".concat(_config.protocol.error).concat(msg));
            }
          ...
          if (id) {
            _this.send("".concat(_config.protocol.result).concat(id, "/").concat(res));
          } else {
            _this.send("".concat(_config.protocol.result).concat(res));
          }
        };
    ...
    if (!data.indexOf(_config.protocol.connect)) {
      var _val = data.substr(_config.protocol.connect.length);
